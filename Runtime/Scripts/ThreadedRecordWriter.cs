...
namespace PLUME
{
    public class ThreadedRecordWriter : IDisposable
    {
        ...
        public ThreadedRecordWriter(SamplePoolManager samplePoolManager, string filepath, string recordIdentifier, ExtraMetadata[] extraMetadata, bool useCompression, int bufferSize = 4096, bool leaveOpen = false)
        {
            _samplePoolManager = samplePoolManager;
            _writer = new RecordWriter(filepath, recordIdentifier, extraMetadata, useCompression, bufferSize, leaveOpen);
